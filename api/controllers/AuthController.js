...
var logger = require('../Log.js').getLoggerWrapper("AuthController");
...
module.exports = {
	...
	process : function(req, res) {
		...
		passport.authenticate('local', function(err, user, info) {
			...
			req.logIn(user, function(err) {
				...
				if (err) {
					logger.info(req, "*** login ERR[" + JSON.stringify(user[0]) + "]");
					openLoginPage(req, res, {type: "danger", contents: "ログインに失敗しました。"});
					return;
				}
				logger.info(req, "*** login OK[" + JSON.stringify(user[0]) + "]");
	...
	logout : function(req, res) {
		var datetime = Utility.getDateTime();
		logger.trace(req, "logout called");
		req.logout();
		if (req.session && req.session.passport) {
			logger.info(req, "*** logout[" + JSON.stringify(req.session.passport) + "]");
